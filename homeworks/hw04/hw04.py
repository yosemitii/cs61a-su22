...
class SmartFridge:
    ...
    def use_item(self, item, quantity):
        ...
        if self.items[item] - quantity <= 0:
            self.items[item] = 0
            print(f'\'Oh no, we need more {item}!\'')
        else:
            self.items[item] -= quantity
...
class VendingMachine:
    ...
    """
    "*** YOUR CODE HERE ***"
    def __init__(self, product, single_price):
    ...
    def add_funds(self, balance):
        if self.stock > 0:
            ...
            print(f'\'Nothing left to vend. Please restock. Here is your ${balance}.\'')

    def vend(self):
        if self.stock == 0:
            print(f'\'Nothing left to vend. Please restock.\'')
        elif self.fund < self.single_price:
            print(f'\'Please update your balance with ${self.single_price - self.fund} more funds.\'')
        elif self.fund == self.single_price:
            print(f'\'Here is your {self.product}.\'')
            self.fund -= self.single_price
            self.stock -= 1
        else:
            print(f'\'Here is your {self.product} and ${self.fund - self.single_price} change.\'')
            self.fund = 0
            self.stock -= 1
