...
def insert_items(lst, entry, elem):
    ...
    for i in range(len(lst)):
        if lst[i+count] == entry:
            lst.append(0)
            for k in range(len(lst)-i - count - 2):
                lst[len(lst) - 1 - k] = lst[len(lst) - 2 - k]
            lst[i+count+1] = elem
            count += 1
    return lst
...
class Coin:
    ...
    def worth(self):
        "*** YOUR CODE HERE ***"
        if self.type == 'Dime':
            return 10 + max(0, Minty.present_year - self.year - 50)
        elif self.type == 'Nickel':
            return 5 + max(0, Minty.present_year - self.year - 50)
