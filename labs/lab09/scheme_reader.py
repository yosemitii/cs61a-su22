...
def scheme_read(src):
    ...
    """
    print("DEBUG:", "Scheme reading src current  :", src.current())
    ...
    if val == 'nil':
        ...
        if src.current() == '(' or src.current() == '\'':
            print("DEBUG: in quote1 curr = ", src.current())
            temp = Pair('quote', Pair(scheme_read(src), nil))
            return temp
        elif src.current() == ')':
            print("DEBUG: in quote2 curr = ", src.current())
            raise SyntaxError('unexpected token: {0}'.format(val))
        else:
            print("DEBUG: in quote3 curr = ", src.current())
            temp = Pair('quote', Pair(src.pop_first(), nil))
            # print("DEBUG: ( returning:", temp)
            return temp
        # END PROBLEM 3
...
def read_tail(src):
    ...
    Pair(2, Pair(3, nil))
    """
    # print("DEBUG:", "reading tail", src.current())
    try:
        if src.current() is None:
            ...
            last = read_tail(src)
            # print("DEBUG:", "first", first, "last", last)
