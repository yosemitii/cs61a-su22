...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        vals = expressions.rest
        new_func = do_lambda_form(Pair(formals, vals), env)
        # if len(formals) != len(vals):
        #     raise SchemeError('Incorrect number of arguments to function call')
        env.define(signature.first, new_func)
...
def do_lambda_form(expressions, env):
    ...
    res = LambdaProcedure(formals, body, env)
    # print('DEBUG: the lambda procedure', res)
    return res
    # END PROBLEM 7
...
def do_or_form(expressions, env):
    ...
    # when it is empty, returns false
    else:
        return False
...
def make_let_frame(bindings, env):
    ...
    while bindings != nil:
        curr = bindings.first
        # validate_form(curr, 2, 3)
        validate_form(curr, 2, 2)
        names = Pair(curr.first, names)
        vals = Pair(curr.rest, vals)
        bindings = bindings.rest
    vals = vals.map(lambda x: scheme_eval(x.first, env))
