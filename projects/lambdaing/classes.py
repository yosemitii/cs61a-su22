...
class TutorCard(Card):
    ...
    def effect(self, opponent_card, player, opponent):
        ...
        if len(player.hand) >= 1:
            card = Card(player.hand[0].name, player.hand[0].attack, player.hand[0].defense)
            # temp = player.hand[0:1] why I can not use this to copy the first card
...
class TACard(Card):
    ...
    def effect(self, opponent_card, player, opponent, arg=None):
        ...
        # BEGIN Problem 5
        best_card = None
        hand_cards_len = len(player.hand)
        max_index = 0

        if hand_cards_len > 0:
            max_power = player.hand[0].power(opponent_card)
            for i in range(hand_cards_len):
                if player.hand[i].power(opponent_card) > max_power:
                    max_index = i
                    max_power = player.hand[i].power(opponent_card)
            best_card = player.hand.pop(max_index)
            print("DEBUG:", best_card)
