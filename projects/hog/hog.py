...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    goal:       The game ends and someone wins when thstrat0 = lambda score, opponent: opponent % 10
is score is reached.
    ...
    while score0 < goal and score1 < goal:
        ...
        if who == 0:
            ...
            # print("||")
            # print(score0)
            num_of_dice0 = strategy0(score0, score1)
            add_score0 = take_turn(num_of_dice0, score0, score1, dice=dice, goal=goal)
            pig0 = pigs_on_prime(score0+add_score0, score1)
            score0 += add_score0 + pig0
        if who == 1:
            # made a lot of errors here
            num_of_dice1 = strategy1(score1, score0)
            add_score1 = take_turn(num_of_dice1, score1, score0, dice=dice, goal=goal)
            pig1 = pigs_on_prime(score1+add_score1, score0)
            score1 += add_score1 + pig1
    # END PROBLEM 5
...
def announce_lead_changes(score0, score1, last_leader=None):
    ...
    if score0 > score1 and last_leader != 0:
        leader, message = 0, f"Player 0 takes the lead by {score0-score1}"
    #do not use if when ther are 3 conditions
    elif score0 < score1 and last_leader != 1:
        leader, message = 1, f"Player 1 takes the lead by {score1-score0}"
    elif score0 == score1:
        leader, message = None, None
    else:
        leader, message = last_leader, None
    # print(leader)
    # print(message)
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    for i in range(1, 11, 1):
        averaged_dice = make_averaged(roll_dice, total_samples=total_samples)
        # average = make_averaged(roll_dice(1, dice), total_samples)
...
def pigs_on_prime_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    score_this_turn = oink_points(score, opponent_score) + pigs_on_prime(score + oink_points(score, opponent_score), opponent_score)
    if score_this_turn >= threshold:
        return 0
